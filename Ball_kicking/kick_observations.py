...
def swing_phase(
    ...
    """[sin, cos] of the swing-cycle phase. (num_envs, 2).

    MUST use the same period as foot_trajectory_swing. Reads env.swing_period
    if set (single source of truth); else falls back to the period arg.
    """
    p = getattr(env, "swing_period", period)
    t = env.episode_length_buf.float() * env.step_dt
    phase = torch.remainder(t, p) / p
